...
public class OnboardeeResourceTest {
	...
	void setUp() {
		...
		this.list.add(new OnboardeeDetails(0, "FSD402","Pawan","9074176787","demo@gmail.com","INT406", "2020-06-07","2020-06-07","FSD401","Mumbai","Done", "Done", "Done"));
		this.list.add(new OnboardeeDetails(1, "FSD402","Pawan","9074176787","demo@gmail.com","INT406", "2020-06-07","2020-06-07","FSD401","Mumbai","Done", "Done", "Done"));
		this.onboardeeDetails = new OnboardeeDetails(1, "FSD402","Pawan","9074176787","demo@gmail.com","INT406", "2020-06-07","2020-06-07","FSD401","Mumbai","Done", "Done", "Done");
	}
	...
	void shouldSaveOnboardeeDetails() throws Exception {
		...
		this.mockMvc.perform(post("/OnboardeeDetails").contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(this.onboardeeDetails)))
				.andExpect(status().isOk());
//				.andExpect(jsonPath("$.location", is("Mumbai")));
	}
	...
	void shouldUpdateOnboardeeDetails() throws Exception {
		...
		this.mockMvc.perform(post("/edit/{id}", 1).contentType(MediaType.APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(this.onboardeeDetails)))
				.andExpect(status().isOk());
//				.andExpect(jsonPath("$.name", is(this.onboardeeDetails.getName())));
	}
	
//	@Test
//	void shouldDeleteOnboardeeDetails() throws Exception {
//
//		when(onboardeeRepo.deleteById(1)).thenReturn(this.onboardeeDetails);
//
//		this.mockMvc.perform(delete("/delete/{id}", 1).andExpect(status().isOk());
////				.andExpect(jsonPath("$.name", is(this.onboardeeDetails.getName())));
//	}
...
//	@Test
//	public void shouldSearchOnboardee() throws Exception {
//		when(onboardeeRepo.findByOnboardeeid("FSD402")).thenReturn(Optional.of(this.onboardeeDetails), Optional.empty());
//		
//		this.mockMvc.perform(get("/search/id")).andExpect(status().isOk());
////			.andExpect(jsonPath("$.name", is("Pawan")));
//	}
